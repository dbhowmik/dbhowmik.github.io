	...
	function registerVideoJsMarkersPlugin(options) {
		...
		function sortMarkersList() {
			...
			markersList.sort(function(a, b) {
				return parseFloat(setting.markerTip.time(a)) - parseFloat(setting.markerTip.time(b));
			});
		}
		...
		function getPosition(marker) {
			return parseFloat(setting.markerTip.time(marker) / player.duration() * 100);
		}
		...
		function setMarkderDivStyle(marker, markerDiv) {
			...
			var ratio = parseFloat(marker.time / player.duration());
			if (ratio < 0 || ratio > 1) {
		...
		function createMarkerDiv(marker) {
			...
			var markerDiv = _video2.default.createEl('div', {}, {
				...
				'data-marker-time': parseFloat(setting.markerTip.time(marker))
			});
			...
			markerDiv.addEventListener('click', function(e) {
				...
				if (!preventDefault) {
					...
					player.currentTime(parseFloat(setting.markerTip.time(markersMap[key])));
				}
			});
		...
		function updateMarkers(force) {
			...
			markersList.forEach(function(marker) {
				...
				var markerTime = parseFloat(setting.markerTip.time(marker));
		...
		function updateBreakOverlay() {
			...
			//console.log("setting.breakOverlay.displayTime: " + setting.breakOverlay.displayTime);
			//console.log("markerTime + setting.breakOverlay.displayTime : " + parseFloat(markerTime + setting.breakOverlay.displayTime));
			...
			*/
			...
			var getNextMarkerTime = function getNextMarkerTime(index) {
				if (index < markersList.length - 1) {
					return parseFloat(setting.markerTip.time(markersList[index + 1]));
				}
			...
			if (currentMarkerIndex !== NULL_INDEX) {
				...
				if (currentTime >= parseFloat(setting.markerTip.time(markersList[currentMarkerIndex])) && currentTime < nextMarkerTime) {
					return;
				}
			...
			if (currentTime < parseFloat(setting.markerTip.time(markersList[0]))) {
				newMarkerIndex = NULL_INDEX;
			} else {
				// look for new index
				for (var i = 0; i < markersList.length; i++) {
					nextMarkerTime = getNextMarkerTime(i);
					if (currentTime >= parseFloat(setting.markerTip.time(markersList[i])) && currentTime < nextMarkerTime) {
						newMarkerIndex = i;
						break;
					}
				}
		...
		player.markers = {
			...
			next: function next() {
				...
				var currentTime = player.currentTime();
				for (var i = 0; i < markersList.length; i++) {
					var markerTime = parseFloat(setting.markerTip.time(markersList[i]));
